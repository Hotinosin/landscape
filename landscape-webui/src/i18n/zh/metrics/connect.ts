export default {
  title: "连接指标",
  all_types: "全部",
  reset: "重置",
  avg: "平均",
  view: {
    connection: "连接",
    iface: "接口",
    src_ip: "源 IP",
    dst_ip: "目的 IP",
  },
  filter: {
    now: "活跃于",
    time: "时间",
    duration: "持续时间",
    src_ip: "源 IP",
    dst_ip: "目的 IP",
    port_start: "源端口",
    port_end: "目的端口",
    proto: "传输协议",
    l3_proto: "IP 类型",
    flow: "分流",
    time_range: "时间范围",
    limit: "数量限制",
    all_status: "不限",
    search_ip: "搜索 IP",
    search_src: "搜索源 IP",
    search_dst: "搜索目的 IP",
    port: "端口",
    create_time: "创建时间",
    gress: "方向",
    gress_egress: "出站",
    gress_ingress: "入站",
    ifindex: "接口索引",
    last_5m: "近 5 分钟",
    last_15m: "近 15 分钟",
    last_1h: "近 1 小时",
    last_6h: "近 6 小时",
    last_24h: "近 24 小时",
    last_3d: "近 3 天",
    custom_range: "自定义时间段",
    limit_100: "限制 100 条",
    limit_500: "限制 500 条",
    limit_1000: "限制 1000 条",
    unlimited: "不限制数量",
  },
  stats: {
    five_sec_sample: "5s 采样",
    refresh_data: "刷新数据",
    query: "查询",
    reset: "重置",
    total_nodes: "共计 {count} 个节点",
    total_active_conns: "总活跃连接",
    total_active_ifaces: "活跃接口",
    total_egress: "总上行",
    total_ingress: "总下行",
    total_history_conns: "历史连接总数",
    total_history_egress: "累计总上传",
    total_history_ingress: "累计总下载",
    page_egress: "本页发送流量",
    page_ingress: "本页接收流量",
    page_egress_pkts: "本页发送包数",
    page_ingress_pkts: "本页接收包数",
    filter_total: "过滤结果总数",
    filter_egress: "过滤结果总上行",
    filter_ingress: "过滤结果总下行",
    egress: "上行",
    ingress: "下行",
    filter_egress_pkts: "过滤结果总出站",
    filter_ingress_pkts: "过滤结果总入站",
    last_summary_time: "最后汇总时间",
  },
  col: {
    ip: "IP 地址",
    src_ip: "源 IP 地址",
    dst_ip: "目的 IP 地址",
    iface: "接口",
    ifindex: "ifindex",
    flow: "所属分流",
    active_conns: "活跃连接",
    egress_rate: "上传速率",
    ingress_rate: "下载速率",
    egress_pps: "上传 PPS",
    ingress_pps: "下载 PPS",
    total_conns: "累计连接",
    total_egress: "累计上传",
    total_ingress: "累计下载",
    egress_pkts: "上传数据包",
    ingress_pkts: "下载数据包",
    port: "端口",
  },
  tip: {
    search_ip: "搜索此 IP",
    precise_filter: "精准筛选此四元组",
    view_history: "查看此连接的历史记录",
    view_live: "查看此连接的实时活跃状态",
    default_flow: "默认分流",
  },
  chart: {
    ingress_total: "入站总量 (Ingress)",
    egress_total: "出站总量 (Egress)",
    ingress_rate: "入站速度 (Ingress)",
    egress_rate: "出站速度 (Egress)",
    ingress_packets_total: "入站数据包 (Ingress)",
    egress_packets_total: "出站数据包 (Egress)",
    ingress_packets_rate: "入站 PPS (Ingress)",
    egress_packets_rate: "出站 PPS (Egress)",
    bytes_axis_total: "数据总量",
    bytes_axis_rate: "数据速度",
    packets_axis_total: "数据包总量",
    packets_axis_rate: "数据包速度 (PPS)",
    second_level: "秒级 (5分钟保留)",
    minute_level: "分钟级 (1天保留)",
    hour_level: "小时级 (7天保留)",
    day_level: "天级 (30天保留)",
    zoom: "框选缩放",
    zoom_back: "撤销缩放",
    reset_zoom: "重置缩放",
  },
  ip_stats: {
    query_ip_ownership: "查询目标 IP 归属",
  },
};
