...
const isManaged = computed(() => selectedSource.value?.kind === "docker");
...
async function refresh() {
  ...
  try {
    ...
    const [docker] = await Promise.allSettled([get_all_docker_networks()]);
...
function restoreUrl() {
  ...
  if (typeof route.query.section === "string") {
    ...
    if (
      ...
      sources.value.get(device.name)?.kind !== "docker"
    ) {
      syncDraft();
      configOpen.value = true;
      configOpen.value = true;
