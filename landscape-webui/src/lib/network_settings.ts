...
export type NetworkSettingsCategory =
  "all" | "wan" | "lan" | "bridge" | "docker" | "plugin" | "unassigned";
export type NetworkSettingsDetailTab = "info" | "config";
export type InterfaceSource = {
  kind: "physical" | "system" | "docker" | "plugin" | "unknown";
  owner?: string;
  available: boolean;
};
...
export function buildInterfaceSources(
  ...
  const pluginByIface = new Map(
    plugins?.map((item) => [item.host_interface, item.name]),
  );
  const sourcesReady = dockerNetworks !== undefined && plugins !== undefined;
  return new Map<string, InterfaceSource>(
    devices.map((device) => {
      const docker = dockerByIface.get(device.name);
      const plugin = pluginByIface.get(device.name);
      if (docker)
        return [
          device.name,
      ...
      if (plugin)
        return [
          device.name,
          { kind: "plugin", owner: plugin, available: true },
        ];
